...
傳回結果為-1，代表「目前位置」的值比「下一個位置」的值小，以[氣泡排序][6]來說，就不用交換。<br>
...
傳回結果為1，代表「目前位置」的值比「下一個位置」的值大，以[氣泡排序][6]來說，就要交換。<br>
...
「目前位置」的值減「下一個位置」的值 > 0，等同於`arr[j] > arr[j + 1]`，二者都是「目前位置」的值「大於」「下一個位置」的值，相減才會是正整數。
...
置 位
   置
```
「下一個位置」的值減「目前位置」的值 <br>
2 - 1 = 1

傳回結果為1，代表「下一個位置」的值比「目前位置」的值大，以[氣泡排序][6]來說，比較結果 > 0要交換。<br>

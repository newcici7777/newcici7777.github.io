...
00000001已經是正整數二進位，左邊第1個bit是0，不用再轉回有正負號的2進位。
...
要把2的補數轉成有正負號的2進位，注意以下箭頭方法。

有正負號的2進位 <- 最左邊第1個bit不變，其它0與1互換 <- 減1 <- 11111111 
...
### 8bit的-128
8bit的-128(2的補數)沒辦法轉回「符號數值表示法」，因為8-bit最大的正整數是127。
{% highlight java linenos %}
public class Test {
  public static void main(String[] args) {
    // -128
